...
def network(prog):
    ...
    for i in range(50):
        inputs.append([i])
        outputs.append([])
        nics.append(Intcode(prog, inputs[i], outputs[i]))
    ...
    while run:
        halted = True
        for i in range(50):
            halted = nics[i].exec() and halted
            j = 0
            while j < len(outputs[i]):
                dest = outputs[i].pop(0)
                x = outputs[i].pop(0)
                y = outputs[i].pop(0)
                if dest == 255:
                    nat_x = x
        ...
        for i in range(50):
            if len(inputs[i]) == 0:
                ...
                idle = False
        print("Idle = ", idle)
        if idle and nat_x is not None and nat_y is not None:
            inputs[0].pop()
            assert len(inputs[0]) == 0
            inputs[0].extend([nat_x, nat_y])
            if (nat_y == nat_last_y) and nat_y != 17541:
                return nat_y
            else:
                nat_last_y = nat_y
                print(nat_y)
